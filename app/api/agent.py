...
@router.get("/runs/{run_id}/actions", dependencies=[Depends(require_api_key)])
def get_agent_run_actions(
    run_id: int,
    db: Session = Depends(get_db),
    agent_service: AgentService = Depends(get_agent_service),
) -> list[dict]:
    actions = agent_service.get_actions(db, run_id)
    if actions is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="agent run을 찾을 수 없습니다.")
    return actions


@router.post("/runs/{run_id}/dry-run", response_model=AgentRunDetail, dependencies=[Depends(require_api_key)])
def dry_run_agent_run(
    run_id: int,
    db: Session = Depends(get_db),
    agent_service: AgentService = Depends(get_agent_service),
) -> dict:
    try:
        run = agent_service.dry_run(db, run_id)
