...
| 환경변수/credential 파일 | `.env`, `.env.*`, `.envrc`, `.npmrc`, `.pypirc`, `.netrc`, `.ssh/`, `.docker/`, `.aws/`, `.gcloud/`, `.azure/`, `.kube/`, `credentials.json`, `client_secret*.json`, `service-account*.json` (`.env.example` 예외) | 제외 |
