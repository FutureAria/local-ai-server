...
| 환경변수/credential 파일 | `.env`, `.env.*`, `.envrc`, `.npmrc`, `.pypirc`, `.netrc`, `.git-credentials`, `.boto`, `.s3cfg`, `.pgpass`, `.ssh/`, `.docker/`, `.aws/`, `.gcloud/`, `.azure/`, `.kube/`, `credentials.json`, `application_default_credentials.json`, `client_secret*.json`, `service-account*.json`, `firebase-adminsdk*.json`, `google-credentials*.json` (`.env.example` 예외) | 제외 |
