...
- [ ] `.env`, `.env.*`, `.envrc`, `.npmrc`, `.pypirc`, `.netrc`, `.ssh/`, `.docker/`, `.aws/`, `.gcloud/`, `.azure/`, `.kube/`, `credentials.json`, `client_secret*.json`, `service-account*.json` 파일이 staging에 없다. 단, placeholder만 담은 `.env.example`은 예외다.
