...
- 실제 API key, `.env`, `.env.*`, key/certificate 파일, SQLite DB, Chroma index, 업로드 파일, 로그, JSONL export를 공개하지 않는다.

## 1. 코드 상태

- [ ] `git status --short --branch`가 의도한 변경만 보여준다.
- [ ] `.env`, `.env.*`가 staging에 없다. 단, placeholder만 담은 `.env.example`은 예외다.
